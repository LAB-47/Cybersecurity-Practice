# -*- coding: utf-8 -*-
"""
NTFS Volume Boot Record (VBR) & Volume Serial Number (VSN) Forensic Parser
...
def parse_vbr_serial(image_path):
    if not os.path.exists(image_path):
        print(f"[ERROR] Evidence file not found: {image_path}", file=sys.stderr)
        sys.exit(1)

    print("=" * 80)
    print("NTFS VOLUME BOOT RECORD (VBR) & VOLUME METADATA EXTRACTION")
    print("=" * 80)
    ...
    with open(image_path, 'rb') as fh:
        ...
        print("\n--- Raw VBR Sector 0 Dump (First 128 Bytes) ---")
        ...
        # Extract 64-bit and 32-bit Volume Serial Number
        ...
        print(f"OEM File System ID:      {oem_id.strip()}")
        ...
        print(f"$MFT Starting Cluster:   LCN {mft_lcn:,}")
        print(f"Raw Serial Bytes (LE):   {' '.join(f'{b:02X}' for b in raw_serial_bytes)}")
        print(f"Full 64-bit NTFS Serial: 0x{serial_64bit:016X}")
        print(f"Standard 32-bit VSN:     {vsn_formatted}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="NTFS VBR & Volume Serial Number Parser")
